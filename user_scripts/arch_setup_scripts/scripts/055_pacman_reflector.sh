...
# --- CACHYOS SYNC ---
sync_cachyos() {
    log_info "CachyOS environment detected. Preserving system pacman configuration."
    if command -v cachyos-rate-mirrors &>/dev/null; then
        manage_pacman_lock
        run_with_spinner "Benchmarking CachyOS mirrors..." cachyos-rate-mirrors || log_warn "cachyos-rate-mirrors returned non-zero."
    else
        log_info "cachyos-rate-mirrors binary not present. Keeping current mirrors."
    fi

    log_info "Synchronizing pacman databases..."
    run_pacman -Syy || log_warn "Pacman database sync returned non-zero."
}
...
sync_arch() {
    ...
    if command -v reflector &>/dev/null; then
        ...
        if run_with_spinner "Fetching 50 fresh HTTPS mirrors (10 threads)..." \
            reflector --latest 50 \
                      --protocol https \
                      --threads 10 \
                      --connection-timeout 2 \
