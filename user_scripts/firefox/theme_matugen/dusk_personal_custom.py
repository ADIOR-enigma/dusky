...
/* Hides the Firefox 140+ sidebar for personal use. */
#sidebar-main {
...
PERSONAL_PREFS = [("toolkit.legacyUserProfileCustomizations.stylesheets", "true")]
...
def restore_pref_line(user_js: Path) -> bool:
    """Remove our exact pref line (never a user-modified value). Returns True if removed."""
    exact = f'user_pref("{PERSONAL_PREFS[0][0]}", {PERSONAL_PREFS[0][1]});'
    ...
    try:
        ...
        print_warn(f"Could not read {user_js}: {e}")
        return False
    kept = [ln for ln in content.splitlines(keepends=True) if ln.strip() != exact]
...
def _browser_processes_running() -> list[str]:
    ...
    try:
        import subprocess
        out = subprocess.run(
            ["pgrep", "-x", "-a", "firefox|librewolf|zen|waterfox|floorp|firedragon"],
...
def restore_profile(profile: Path) -> bool:
    ...
    if chrome_dir.is_dir():
        ...
        if not _dusky_chrome_present(chrome_dir):
            if restore_pref_line(profile / "user.js"):
                removed = True
    return removed
