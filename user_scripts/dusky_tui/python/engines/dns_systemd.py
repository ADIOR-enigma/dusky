...
class SystemdDnsEngine(BaseEngine):
    ...
    def _check_port_53_conflict(self) -> str | None:
        ...
        """
        try:
            # -H: no header, -l: listening, -t: tcp, -u: udp, -n: numeric, -p: processes
            res = subprocess.run(["ss", "-H", "-ltunp"], capture_output=True, text=True)
            for line in res.stdout.splitlines():
                if ":53 " in line or ":53\t" in line or line.endswith(":53"):
                    if "systemd-resolve" in line:
                        continue
                    
                    # Match wildcard (0.0.0.0, *), IPv6 wildcard ([::]), or loopbacks
                    if re.search(r'\b(0\.0\.0\.0:53|\*:53|\[::\]:53|\[::ffff:0\.0\.0\.0\]:53|127\.0\.0\.\d+:53)\b', line):
                        match = re.search(r'users:\(\("([^"]+)"', line)
                        return match.group(1) if match else "Unknown DNS Process"
        except OSError:
            pass
    ...
    def _sync_resolv_conf_symlink(self, stub_mode: str) -> None:
        ...
        try:
            current_target = etc_resolv.readlink() if etc_resolv.is_symlink() else None
            ...
            print(f"[DnsEngine] Symlink Sync Error: {e}")
    ...
    def write_batch(self, changes: list[tuple[str, str, str, str]]) -> tuple[bool, str, str]:
        ...
        try:
            ...
            if subprocess.run(["command", "-v", "resolvectl"], capture_output=True, shell=True).returncode == 0:
