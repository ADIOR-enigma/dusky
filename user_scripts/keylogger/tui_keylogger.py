...
SCHEMA = {
    ...
    1: [
        ...
        ConfigItem(
            ...
            options=[
                "/tmp",
                "/temp",
                "/tmp/dusky_keylogger_transcripts",
                "~/tmp/dusky_transcripts",
                "~/.cache/dusky_keylogger/transcripts",
            ],
            hints=[
                "Standard tmpfs (cleared on reboot, 1777, recommended)",
                "Alt tmp (if you prefer /temp, auto-created, needs 1777)",
                "Subdir in /tmp (auto-created, inherits tmpfs)",
                "In HOME tmp (persistent home, not cleared — not ephemeral, for testing)",
                "XDG cache (not cleared on reboot — for persistent cache)",
            ],
            group="Ephemeral",
            extended_help="**Transcript Directory**\n\nEphemeral — where `dusky text --period today` writes `dusky-typed-<period>-<date>.[txt|md]` (default `/tmp`, cleared on reboot, `0600`).\n- Supports `~`, `$HOME`, relative (relative → `Path.home()/...`), and env `DUSKY_TRANSCRIPT_DIR` (highest priority).\n- Auto-created on first `dusky text` if missing (`mkdir -p`). Fresh install dir doesn't exist — TUI ensures creation.\n- If `/temp` missing, `mkdir -p /temp` needs sudo (`sudo mkdir -p /temp && sudo chmod 1777 /temp && sudo chown $USER:$USER /temp`) — TUI shows error hint if it fails.\n- Persistent stats stay in `data_dir` regardless.\n\nIntelligent: `/tmp`/`/temp` leaf not chmodded (keep `1777`); other dirs set `0700`.",
    ...
    2: [
        ...
        ConfigItem(
            ...
            scope="DEFAULT",
            type_="action",
            default="bash -c 'systemctl status dusky_keylogger --no-pager -l -n 50; echo \"---\"; echo \"Data: $HOME/.local/share/dusky-keylogger (or configured data_dir)\"; ls -lh \"$HOME/.local/share/dusky-keylogger/\" 2>/dev/null | head -n 20; echo \"---\"; echo \"Config: $HOME/.config/dusky/settings/keylogger/config.json (new) or $HOME/.config/dusky-keylogger/config.json (old, auto-migrated)\"; ls -l \"$HOME/.config/dusky/settings/keylogger/config.json\" \"$HOME/.config/dusky-keylogger/config.json\" 2>/dev/null | head; read -p \"Press Enter...\"'",
            group="Diagnostics",
            extended_help="**Status**\n\nShows `systemctl status` plus `ls` of persistent data dir and both config paths (new canonical + old fallback, auto-migrated).",
            force_interactive=True,
        ),
        ConfigItem(
    ...
    3: [
        ...
        ConfigItem(
            ...
            extended_help="**Dashboard**\n\nLaunches `dusky dashboard` (Textual) — live cards (today/week/month/all), hourly sparkline, top keys bar, recent events. Reads `keys.db` via `query_only` (WAL concurrent with daemon). `q` to quit, `r` to refresh.",
            force_interactive=True,
        ),
        ConfigItem(
            label="Stats Today",
    ...
    4: [
        ...
        ConfigItem(
            ...
            preset_payload={
                ...
                "DEFAULT.data_dir": "~/.local/share/dusky-keylogger",
                "DEFAULT.transcript_dir": "/tmp",
                "DEFAULT.transcript_format": "text",
            },
            extended_help="**Balanced**\n\nDefault: persistent ON (`~/.local/share/dusky-keylogger`, `0.5s` flush, `info`), ephemeral ON (`/tmp`, `text`). Recommended for daily use.",
        ),
        ConfigItem(
            label="Full Verbose",
            key="preset_verbose",
